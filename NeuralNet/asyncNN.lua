  ...
  return insertTasks(
    forEach( "NNet:backProp - layers", function( layerNum )
      ...
      insertTasks(
        ...
        -- },{
        --   label = "NNet:backProp - queue compute errors",
        --   func = function()
        --     if #errorValues == 0 then
        --       insertTasks( --called after outputs set
        --         forEach("NNet:backProp - compute errors", function( i, output )
        --           local target = targets[ i ]
        --           errorValues[ i ] = target - output 
        --         end, ipairs, outputs)
        --       )
        --     end
        --     return true
        ...
        -- {
        --   label = "NNet:backProp - compute targets",
        --   func = function()
        --     if layerNum == #this.layers then return true end
        --     local nextLayer = this.layers[ layerNum+1 ]
        --     local oldTargets = targets
        --     errorValues = {}
        --     targets = {}
        --     insertTasks(
        --       forEach( "NNet:backProp - compute target value", function( targetIndex, _ )
        --         targets[ targetIndex ] = 0
        --         insertTasks(
        --           forEach( "NNet:backprop - computeTargets", function( n, neuron ) 
        --             targets[ targetIndex ] = targets[ targetIndex ] + neuron.value * neuron.weights[ targetIndex ] * (oldTargets[n] - neuron.value)
        --           end, ipairs, nextLayer )
        --         )
        --       end, ipairs, layer )
        --     )
        --     return true
          ...
          insertTasks(
            {
              ...
              func = function()
                ...
                neuron:backProp( layerInputs, errorValue, layer.learningRate or self.config.learningRate or .005 ) 
                return true
              end
            },{
...
function Neuron:feedForward( withInputs )
	if #withInputs ~= #self.weights then
		error("input / weights mismatch [got "..#withInputs.." expected "..#self.weights.."]",2) 
	end
...
function train()
  ...
  local epochs = data.epocs or 1
  insertTasks(
    forEach("plasma - train - epoch", function( epoch )
      plasma.nnet:train( features, labels )
    end, range, 1, epochs ),
    {
